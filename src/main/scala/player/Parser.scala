...
object Parser extends RegexParsers {
  ...
  private val bonus: Parser[Bonus] = """[-+]\d+""".r ^^ { (bonus: String) =>
    Bonus(bonus.toInt)
  }
  ...
  private val armor: Parser[Armor] =
    bonus ~ armorType ~ "[" ~ number ~ "]" ^^ {
      case bonus ~ armorType ~ _ ~ _ ~ _ => Armor(armorType, bonus)
    }

  private val weapon: Parser[Weapon] =
    "a" ~ bonus ~ "," ~ bonus ~ weaponType ^^ {
      case _ ~ plusToHit ~ _ ~ plusDamage ~ weaponType =>
        Weapon(weaponType, plusToHit, plusDamage)
    } |
      number ~ bonus ~ "," ~ bonus ~ pluralWeaponType ^^ {
        case quantity ~ plusToHit ~ _ ~ plusDamage ~ weaponType =>
          Weapon(quantity, weaponType, plusToHit, plusDamage)
      } |
      "a" ~ weaponType ^^ { case _ ~ weaponType => Weapon(weaponType) } |
      number ~ pluralWeaponType ^^ { case quantity ~ missileType =>
        Weapon(quantity, missileType)
      }
  ...
  private val scroll: Parser[Scroll] =
    "a scroll entitled: '" ~ """\w+(\s\w+)*""".r ~ "'" ^^ {
      case _ ~ title ~ _ => Scroll(title)
    }

  private val food: Parser[Food] =
    "some food" ^^^ Food.Rations(1) |
      number ~ "rations of food" ^^ { case quantity ~ _ =>
  ...
  private val event: Parser[Event] =
    "was wearing" ~ armorType ^^ { case _ ~ armorType =>
      Event.WasWearing(armorType)
    } |
      item ~ "(" ~ slot ~ ")" ^^ { case item ~ _ ~ slot ~ _ =>
        Event.PickedUp(item, slot)
      } |
      slot ~ ")" ~ item ^^ { case slot ~ _ ~ item =>
        Event.Identified(slot, item)
      } |
      "a cloak of darkness falls around you" ^^^ Event.Quaffed(
        PotionPower.Blindness
      ) |
      "a teleport trap" ^^^ Event.FoundTrap(TrapType.Teleport) |
      "a bear trap" ^^^ Event.FoundTrap(TrapType.Bear) |
      "a strange white mist envelops you and you fall asleep" ^^^ Event
        .Triggered(TrapType.Sleep) |
      "a sleeping gas trap" ^^^ Event.FoundTrap(TrapType.Sleep) |
      "a small dart just hit you in the shoulder" ^^^ Event.Triggered(
        TrapType.Dart
      ) |
      "a poison dart trap" ^^^ Event.FoundTrap(TrapType.Dart) |
      "a rust trap" ^^^ Event.FoundTrap(TrapType.Rust) |
      "a gush of water hits you on the head" ^^^ Event.Triggered(
        TrapType.Rust
      ) |
      "defeated the something" ^^^ Event.Killed(None) |
      "defeated the" ~ monsterType ^^ { case _ ~ monsterType =>
        Event.Killed(Some(monsterType))
      } |
      "dropped" ~ item ^^ { case _ ~ item => Event.Dropped(item) } |
      "everything looks SO boring now" ^^^ Event.Unhallucinate |
      "hmm, this potion tastes like slime-moldjuice" ^^^ Event.Quaffed(
        PotionPower.SeeInvisible
      ) |
      ...
      "oh wow, everything seems so cosmic" ^^^ Event.Quaffed(
        PotionPower.Hallucination
      ) |
      ...
      "the" ~ bolt ~ "misses" ^^ { case _ ~ bolt ~ _ =>
        Event.BoltMisses(bolt)
      } |
      "the" ~ bolt ~ "hits the" ~ monsterType ^^ {
        case _ ~ bolt ~ _ ~ monsterType =>
          Event.BoltHitsMonster(bolt, monsterType)
      } |
      "the something hit" ^^^ Event.HitBy(None) |
      "the" ~ monsterType ~ "hit" ^^ { case _ ~ monsterType ~ _ =>
        Event.HitBy(Some(monsterType))
      } |
      "the something misses" ^^^ Event.HitBy(None) |
      "the" ~ monsterType ~ "misses" ^^ { case _ ~ monsterType ~ _ =>
        Event.MissedBy(Some(monsterType))
      } |
      "the" ~ weaponType ~ "hit" ^^ { case _ ~ weaponType ~ _ =>
        Event.ThrownHit(weaponType)
      } |
      "the" ~ weaponType ~ "misses" ^^ { case _ ~ weaponType ~ _ =>
        Event.ThrownMisses(weaponType)
      } |
      "the monster appears confused" ^^^ Event.ConfusedMonster |
      "the rust vanishes instantly" ^^^ Event.RustVanishes |
      "the gaze of the" ~ monsterType ~ "has confused you" ^^ {
        case _ ~ monsterType ~ _ => Event.ConfusedBy(monsterType)
      } |
      "the veil of darkness lifts" ^^^ Event.Unblind |
      "the scroll turns to dust as you pick it up" ^^^ Event.ScrollDust |
      "the" ~ item ~ "vanishes as it hits the ground" ^^ { case _ ~ item ~ _ =>
        Event.ItemVanishes(item)
      } |
      "the monsters around you freeze" ^^^ Event.HoldMonsters |
      "the monster freezes" ^^^ Event.HoldMonsters |
      "wait, that's a" ~ monsterType ~ "!" ^^ { case _ ~ monsterType ~ _ =>
        Event.Undisguise(monsterType)
      } |
      ...
      "welcome to level" ~ number ^^ { case _ ~ newLevel =>
        Event.LevelUp(newLevel)
      } |
      "you hit" ^^^ Event.Hit |
      "you miss" ^^^ Event.Miss |
      ...
      "you begin to feel much better" ^^^ Event.Quaffed(
        PotionPower.ExtraHealing
      ) |
      "you feel a strange sense of loss" ^^^ Event.Read(
        ScrollPower.HoldMonster
      ) |
      "you feel a wrenching sensation in your gut" ^^^ Event.Upstairs |
      "you feel stronger now, what bulging muscles!" ^^^ Event.Quaffed(
        PotionPower.IncreaseStrength
      ) |
      "you feel very sick now" ^^^ Event.Quaffed(PotionPower.Poison) |
      "you suddenly feel much more skillful" ^^^ Event.Quaffed(
        PotionPower.RaiseLevel
      ) |
      "you start to float in the air" ^^^ Event.Quaffed(
        PotionPower.Levitation
      ) |
      "you float gently to the ground" ^^^ Event.Unlevitate |
      "you feel yourself moving much faster" ^^^ Event.Quaffed(
        PotionPower.HasteSelf
      ) |
      ...
      "you feel as though someone is watching over you" ^^^ Event.Read(
        ScrollPower.RemoveCurse
      ) |
      "you feel in touch with the universal oneness" ^^^ Event.Read(
        ScrollPower.RemoveCurse
      ) |
      ...
      "your" ~ weaponType ~ "glows" ~ colour ~ "for a moment" ^^ {
        case _ ~ weaponType ~ _ ~ _ ~ _ => Event.EnchantWeapon(weaponType)
      } |
      "you hear a high pitched humming noise" ^^^ Event.AggravateMonster |
      "you hear a maniacal laughter in the distance" ^^^ Event.Read(
        ScrollPower.ScareMonster
      ) |
      "you hear a faint cry of anguish in the distance" ^^^ Event.Read(
        ScrollPower.CreateMonster
      ) |
  ...
  private val messageLine: Parser[MessageLine] =
    event.* ~ "-more-" ^^ { case events ~ _ => MessageLine.More(events) } |
      (event ~ """\s*""".r).* ^^ { (events: Seq[Event ~ String]) =>
        MessageLine.Message(events.map(_._1))
      }
  ...
  private val statusLine: Parser[StatusLine] =
    "Level:" ~ number ~ "Gold:" ~ number ~ "Hp:" ~ number ~ "(" ~ number ~ ")" ~ "Str:" ~ number ~ "(" ~ number ~ ")" ~ "Arm:" ~ number ~ "Exp:" ~ number ~ "/" ~ number ~ hunger ^^ {
      case _ ~ level ~ _ ~ gold ~ _ ~ hp ~ _ ~ maxhp ~ _ ~ _ ~ str ~ _ ~ maxstr ~ _ ~ _ ~ armor ~ _ ~ exp ~ _ ~ expLevel ~ hunger =>
        StatusLine(
          level,
          gold,
          hp,
  ...
  private val inventoryLine: Parser[InventoryScreenLine] =
    """[^a-z]*""".r ~ slot ~ ")" ~ armor ~ "being worn" ^^ {
      case _ ~ slot ~ _ ~ armor ~ _ => InventoryScreenLine.Wearing(slot, armor)
    } |
      """[^a-z]*""".r ~ slot ~ ")" ~ weapon ~ "in hand" ^^ {
        case _ ~ slot ~ _ ~ wieldable ~ _ =>
          InventoryScreenLine.Wielding(slot, wieldable)
      } |
      """[^a-z]*""".r ~ slot ~ ")" ~ item ^^ { case _ ~ slot ~ _ ~ item =>
        InventoryScreenLine.InventoryItem(slot, item)
      }
}

enum InventoryScreenLine(val slot: Slot, val item: Item) {
  case Wearing(override val slot: Slot, armor: Armor)
      extends InventoryScreenLine(slot, armor)
  case Wielding(override val slot: Slot, wieldable: Wieldable)
      extends InventoryScreenLine(slot, wieldable)
